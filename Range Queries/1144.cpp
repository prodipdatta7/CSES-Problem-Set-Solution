 ...
 *
 *  author  :   prodipdatta7
 *  created :   Friday 08-May, 2020  11:55:52 AM
**/
...
int main() {
	...
	while (test--) {
		...
		while (q--) {
			scanf("%c %d %d", &c, &k, &x) ;
			if (c == '!') {
				os.erase(os.find(make_pair(a[k], k))) ;
				os.insert(make_pair(x, k)) ;
				a[k] = x ;
			}
			else {
				int res = os.order_of_key(make_pair(x + 1, 0)) - os.order_of_key(make_pair(k, 0)) ;
				printf("%d\n", res);
			}
			getchar() ;
		}
