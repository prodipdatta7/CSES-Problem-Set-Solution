 ...
 *
 *  author  :   prodipdatta7
 *  created :   Friday 08-May, 2020  12:15:31 PM
**/
...
struct segment {
	...
	void upd(int i , int b , int e , int p, pii v) {
		...
		tree[i].ss = tree[i << 1].ss + tree[i << 1 | 1].ss ; 
	}

	pii query(int i , int b , int e, int p) {
		if (b == e)return make_pair(b, tree[i].ff) ;
		int mid = (b + e) >> 1 ; 
		if(tree[i << 1].ss >= p)return query(i << 1, b, mid, p) ; 
		else return query(i << 1 | 1, mid + 1, e, p - tree[i << 1].ss) ; 
	}

	void solve() {
		int n ; Int(n) ;
		tree.assign(n << 2, make_pair(0, 0)) ; 
		for (int i = 1 ; i <= n ; ++i) {
			int x ; Int(x) ; 
			upd(1, 1, n, i, make_pair(x, 1)) ;
		}
		for(int i = 1 ; i <= n ; ++i){
			int x ; Int(x) ; 
			pii p = query(1, 1, n, x) ; 
			cout << p.ss << " \n"[i == n] ; 
			upd(1, 1, n, p.ff, make_pair(0, 0)) ;
		}
	}
} seg;
